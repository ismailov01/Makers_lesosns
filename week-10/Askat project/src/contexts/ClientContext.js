...
const INIT_STATE = {
  phones: null,
  phoneDetails: null,
  phoneCountInCart: JSON.parse(localStorage.getItem("cart"))
...
const reducer = (state = INIT_STATE, action) => {
  switch (action.type) {
    ...
    case "ADD_AND_DELETE_PHONE_IN_CART":
      return { ...state, phoneCountInCart: action.payload };
    case "GET_CART":
...
const ClientContextProvider = (props) => {
  ...
  const addAndDeletePhoneInCart = (phone) => {
    ...
    let chekArr = cart.phones.filter((item) => {
  ...
  return (
    <clientContext.Provider
      value={{
        ...
        phoneCountInCart: state.phoneCountInCart,
